...
-- Create frontend role if it doesn't exist
DO
$do$
BEGIN
   IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'frontend') THEN
      CREATE ROLE frontend WITH LOGIN PASSWORD 'frontend';
   END IF;
END
$do$;
